...
async function handleFaceQRCode(page, log, mainWindow) {
  ...
  while (true) {
    ...
    if (qrVisible && !lastQRCodeVisible) {
      ...
      // 启动倒计时
      remain = 120;
      mainWindow.webContents.send('qrcode-timer', remain);
      // 先清理旧定时器
      if (timer) {
        clearInterval(timer);
        timer = null;
...
async function startAutomation(config) {
  ...
  while (true) {
    ...
    try {
      ...
      log('未能找到验证码图片，详细错误：' + (e && e.stack ? e.stack : e));
      return;
    }
    ...
    if (feedbackText === '登录成功' || currentUrl === successUrl) {
      ...
      try {
        ...
        try {
          ...
          log('课件列表推送失败：' + (e && e.stack ? e.stack : e));
        }
        ...
        log('后续自动化操作出错：' + (e && e.stack ? e.stack : e));
      }
